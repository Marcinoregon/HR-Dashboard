...
function renderExercises() {
    ...
    // ── Student name bar + unlock bar + unlocked banner ──
    const downloadBar = `
        <div class="exercises-student-bar" id="exercises-student-bar">
            <div class="exercises-student-bar-text">
                <div class="exercises-student-bar-title">👤 Student Information</div>
                <div class="exercises-student-bar-desc">Enter your name — it will appear on your downloaded submission.</div>
            </div>
            <div class="exercises-student-bar-inputs">
                <input
                    class="exercises-student-input"
        ...
        <div class="solutions-unlock-bar" id="solutions-unlock-bar">
            <div class="solutions-unlock-bar-left">
                <div class="solutions-unlock-bar-icon">🔐</div>
                <div class="solutions-unlock-bar-text">
                    <div class="solutions-unlock-bar-title">Unlock Model Answers</div>
                    <div class="solutions-unlock-bar-desc">Enter the instructor code below to reveal solutions after submitting your answers.</div>
                </div>
            </div>
            <div class="solutions-unlock-bar-right">
                <input
                    class="solutions-unlock-input"
                    id="inline-code-input"
                    type="password"
                    maxlength="6"
                    placeholder="Enter code"
                    autocomplete="off"
                    onkeydown="if(event.key==='Enter') submitInlineCode()"
                />
                <button class="solutions-unlock-submit" onclick="submitInlineCode()">
                ...
                <span class="solutions-unlock-error" id="inline-code-error"></span>
            </div>
        </div>
        <div class="solutions-unlocked-banner" id="solutions-unlocked-banner">
            ...
            Solutions unlocked for this session. Model answers are now visible.
        </div>
    `;
    ...
    for (const ex of exercises) {
        let mcqHtml = '';
        if (ex.mcqs) {
            const questionItems = ex.mcqs.map((mcq, qi) => {
                questionCounter++;
                const globalQ = questionCounter;
                return `
                <div class="mcq-card" id="mcq-${ex.number}-${qi}">
                    <div class="mcq-question">${globalQ}. ${mcq.q}</div>
        ...
        <div class="exercise-card">
            ...
            <div class="exercise-answer-area">
                ...
                <div class="exercise-answer-btns">
                    ...
                    <button class="exercise-feedback-btn locked" id="ex-fb-btn-${ex.number}" onclick="requestSolutionUnlock(${ex.number})">🔒 View Model Answer</button>
    ...
    const downloadBarBottom = `
        ...
        <div class="exercises-download-bar" id="exercises-download-bar-bottom">
            <div class="exercises-download-bar-text">
                ...
                <div class="exercises-download-bar-desc">Save a copy of all your exercise answers as a text file to submit to your instructor.</div>
            </div>
            <button class="exercises-download-btn" onclick="downloadExerciseAnswers()">
    ...
    grid.innerHTML = downloadBar + exerciseCards + downloadBarBottom;
    ...
    exercises.forEach(ex => {
        ...
        if (saved) {
            ...
            // Show locked feedback button (requires code to unlock)
    ...
    // If already unlocked this session, apply unlocked state
...
function submitExercise(num) {
    ...
    // Show the feedback button in locked state
    const fbBtn = document.getElementById(`ex-fb-btn-${num}`);
    fbBtn.classList.add('visible');
    // If already unlocked, switch to unlocked mode immediately
    if (solutionsUnlocked) {
        fbBtn.classList.remove('locked');
        fbBtn.textContent = '💡 View Model Answer';
        fbBtn.onclick = () => toggleExerciseFeedback(num);
    }
...
let pendingUnlockExNum = null;

function requestSolutionUnlock(exNum) {
    pendingUnlockExNum = exNum;
    const modal = document.getElementById('solutionModal');
    const input = document.getElementById('solutionCodeInput');
    const err = document.getElementById('solutionCodeError');
...
function submitSolutionCode() {
    ...
    if (code === '99') {
        ...
        err.textContent = 'Incorrect code. Please check with your instructor.';
    }
}

function submitInlineCode() {
    const input = document.getElementById('inline-code-input');
    const err = document.getElementById('inline-code-error');
    if (!input) return;
    const code = input.value.trim();
...
function applyUnlockedState() {
    // Hide the inline unlock bar, show unlocked banner
    const unlockBar = document.getElementById('solutions-unlock-bar');
    if (unlockBar) unlockBar.style.display = 'none';
    const banner = document.getElementById('solutions-unlocked-banner');
    ...
    for (let n = 1; n <= 6; n++) {
        ...
        // Update or show the feedback button in unlocked state
        const fbBtn = document.getElementById(`ex-fb-btn-${n}`);
        if (fbBtn) {
            fbBtn.classList.remove('locked');
            fbBtn.classList.add('visible');
            fbBtn.textContent = '🔽 Hide Model Answer';
            const num = n;
            fbBtn.onclick = () => toggleExerciseFeedback(num);
        }
    }
    ...
    for (let n = 1; n <= 6; n++) {
        ...
        while (true) {
            ...
            // Only reveal if student hasn't already answered
            if (!card.classList.contains('answered')) {
                const options = card.querySelectorAll('.mcq-option');
                let correctIdx = -1;

                // Parse correct index from the onclick attribute
                options.forEach((opt, oi) => {
                    const onclick = opt.getAttribute('onclick');
                    if (onclick) {
                        const match = onclick.match(/checkMCQ\(\d+,\s*\d+,\s*(\d+),\s*(\d+)\)/);
                        if (match) {
                            const thisOi = parseInt(match[1]);
                            const correct = parseInt(match[2]);
                            if (thisOi === correct) correctIdx = oi;
                        }
                    }
                });

                // Mark correct option green and disable all
                options.forEach((opt, oi) => {
                    opt.disabled = true;
                ...
                const fb = document.getElementById(`mcq-fb-${n}-${qi}`);
                if (fb && correctIdx >= 0) {
                    const explanation = options[correctIdx]?.getAttribute('data-explanation') || '';
                    fb.innerHTML = `
                        <div class="mcq-fb-indicator mcq-fb-correct">✅ Correct Answer Revealed</div>
...
// ===== DOWNLOAD EXERCISE ANSWERS =====
function downloadExerciseAnswers() {
    const exerciseTitles = [
        'Exercise 1 — The "New Hire" Audit',
    ...
    const studentName = [firstName, lastName].filter(Boolean).join(' ') || 'Not provided';

    // Helper: get MCQ score for an exercise from the DOM score element
    function getMCQScore(exNum) {
        ...
        if (scoreEl && scoreEl.textContent.includes('/')) {
            ...
            if (!isNaN(parts[0]) && !isNaN(parts[1]) && parts[1] > 0) {
                return { correct: parts[0], total: parts[1] };
            }
        }
        // Fallback: check localStorage state
        const state = JSON.parse(localStorage.getItem(`macky_mcq_${exNum}`) || '{}');
        if (!Object.keys(state).length) return null;
        let correct = 0, total = 0;
        Object.keys(state).forEach(qIdx => {
            total++;
            const selIdx = state[qIdx];
            const optBtn = document.getElementById(`mcq-opt-${exNum}-${qIdx}-${selIdx}`);
            if (optBtn) {
                const m = (optBtn.getAttribute('onclick') || '').match(/checkMCQ\(\d+,\s*\d+,\s*(\d+),\s*(\d+)\)/);
                if (m && parseInt(m[1]) === selIdx && parseInt(m[2]) === selIdx) correct++;
            }
        });
    ...
    for (let n = 1; n <= 6; n++) {
        ...
        doc += `----------------------------------------------------------------\n`;
        doc += `${exerciseTitles[n - 1]}\n`;
        doc += `----------------------------------------------------------------\n\n`;

        doc += `YOUR ANSWER:\n${savedAnswer || '(No answer submitted yet.)'}\n\n`;

        if (mcq) {
            const pct = Math.round(mcq.correct / mcq.total * 100);
            doc += `KNOWLEDGE CHECK: ${mcq.correct} / ${mcq.total} correct (${pct}%)\n`;
            totalCorrect += mcq.correct;
            totalMCQ    += mcq.total;
        } else {
            doc += `KNOWLEDGE CHECK: Not attempted\n`;
        }
        doc += '\n';
    }
    ...
    doc += '================================================================\n';
    if (totalMCQ > 0) {
        const overallPct = Math.round(totalCorrect / totalMCQ * 100);
        doc += `  OVERALL KNOWLEDGE CHECK: ${totalCorrect} / ${totalMCQ} correct (${overallPct}%)\n`;
    }
    doc += '  END OF SUBMISSION\n';
    doc += '  MAN 6136 — HR Analytics | Mr. Macky\'s Cajun Cuisine Dashboard\n';
    doc += '================================================================\n';
    ...
    a.href = url;
    const safeName = studentName !== 'Not provided' ? '_' + studentName.replace(/\s+/g, '_') : '';
    a.download = `HR_Analytics_Exercises${safeName}_${now.getFullYear()}-${String(now.getMonth()+1).padStart(2,'0')}-${String(now.getDate()).padStart(2,'0')}.txt`;
            ...
            questions: [
                {
                    ...
                    answer: [
                        { heading: '📊 Expected Output', points: ['<strong>Large Market avg engagement:</strong> <span class="worked-number">~3.86</span>', '<strong>Medium Market avg engagement:</strong> <span class="worked-number">~3.66</span>', '<strong>Difference:</strong> <span class="worked-number">~+0.20</span> in favor of Large Markets', '<strong>t-Statistic:</strong> approximately <span class="worked-number">2.79</span>', '<strong>p-Value:</strong> ~0.006 — <strong>statistically significant</strong> (p &lt; 0.01)'] },
                        { heading: 'How to Interpret', points: ['The gap in engagement between Large and Medium markets is real — it\'s not random variation. Large market employees consistently score higher across the 9 core engagement dimensions.', 'Possible reasons: Larger units may have more resources, better training infrastructure, more experienced managers, and stronger career development pathways.', 'Important caveat: statistical significance does not mean the gap is <em>large</em> enough to matter operationally. A ~0.20-point difference on a scale where scores cluster between 3–4 is modest, even if reliable.'] },
                        { heading: 'What This Paradox Sets Up', points: ['In Practice Question 2, you\'ll find that Large Market units also have <em>higher turnover</em>. If engagement drives retention, why do more-engaged large-market employees leave at higher rates?', 'The answer points to confounding: large markets employ more college-age workers with higher natural turnover regardless of engagement. This is a classic omitted variable problem.'] }
                    ],
                ...
                },
                {
                    ...
                    answer: [
                        { heading: '📊 Expected Output', points: ['<strong>Large Market avg turnover rate:</strong> <span class="worked-number">~85–86%</span>', '<strong>Medium Market avg turnover rate:</strong> <span class="worked-number">~85–86%</span>', '<strong>t-Statistic:</strong> approximately <span class="worked-number">−1.1</span>', '<strong>p-Value:</strong> ~0.26 — <strong>not statistically significant</strong>'] },
                        { heading: 'What the Result Reveals', points: ['Unlike engagement, turnover rates are nearly <em>identical</em> between Large and Medium market units (~85–86% each). The t-test finds no significant difference between the two groups.', 'This is itself a meaningful finding: despite having higher average engagement, Large Market units do <em>not</em> show meaningfully lower turnover — suggesting engagement alone does not drive retention differences at the market level.', 'This points to confounding: labor market composition, job type, and seasonal patterns likely dominate turnover behavior across all markets, making market type a weak predictor of turnover rates.'] },
                        { heading: 'Key Takeaway for Case 2', points: ['A non-significant result is still informative. It tells us that market type does <em>not</em> predict turnover — even though it does predict engagement.', 'This motivates multivariate OLS: to control for unit-level factors (payroll, headcount, engagement) that may predict turnover better than market type alone.'] }
                    ],
            ...
            questions: [
                {
                    ...
                    answer: [
                        { heading: '📊 Expected Output', points: ['<strong>R-Squared (2-predictor model):</strong> approximately <span class="worked-number">0.38 – 0.39</span>', 'This means performance rating and market type together explain roughly <strong>38–39% of the variation</strong> in manager wages.', 'The remaining ~61% is explained by factors not in this model (experience, tenure, negotiating history, school attended, etc.)'] },
                        { heading: 'Is This Good?', points: ['For social science / HR data, an R² of 0.38–0.39 is <strong>moderate-to-good</strong> for a 2-variable model. Two predictors capturing nearly 40% of wage variance is meaningful.', 'When you add more variables (experience, gender, race), R² rises further in Case Study 2 — a meaningfully better fit.', 'R² is a measure of <em>explanatory power</em>, not of whether the model\'s coefficients are accurate. A model with moderate R² can still have highly significant, valid coefficients.'] },
                        { heading: 'Common Mistake to Avoid', points: ['R² is a measure of <em>explanatory power</em>, not of whether the model\'s coefficients are accurate. A model with R² = 0.10 can still have highly significant, valid coefficients.', 'Focus on R² for comparing models. Focus on p-values for evaluating individual predictors.'] }
                    ],
                ...
                },
                {
                    ...
                    answer: [
                        { heading: '📊 Expected Output', points: ['<strong>Performance Rating Coefficient:</strong> approximately <span class="worked-number">+$1,800 – $2,100</span>', '<strong>p-Value:</strong> &lt; 0.01 — <strong>statistically significant</strong>', '<strong>Interpretation:</strong> Holding market type constant, each 1-point increase in performance rating is associated with approximately $1,900 more in annual wages.'] },
                        { heading: 'How to Use This', points: ['Coefficient = $1,923 (from full Case 2 model). A manager rated 4 vs. 3 earns ~$1,923 more per year, all else equal.', 'A manager rated 5 vs. 3 earns ~$3,846 more — the coefficient scales linearly.', 'This means the company does reward performance — but the pay-for-performance premium (~$2K) is small relative to market type ($10K premium for Large markets).'] },
                        { heading: 'What "Holding Constant" Means', points: ['The coefficient tells you the effect of performance <em>among managers in the same market type</em>. This is what makes multivariate OLS powerful: it isolates the unique contribution of each variable.', 'If you ran a simple T-test of high-performers vs. low-performers, market type would confound the result (high-performing managers might cluster in large markets which pay more anyway).'] }
                    ],
                ...
                },
                {
                    ...
                    answer: [
                        { heading: '📊 Expected Output', points: ['<strong>Avg Engagement Coefficient:</strong> approximately <span class="worked-number">+$100,300</span> per unit', '<strong>p-Value:</strong> ~0.037 — <strong>statistically significant at α = 0.05</strong>', '<strong>Calculation for 0.5-point improvement:</strong> $100,300 × 0.5 = <span class="worked-number">~$50,150 per unit</span>'] },
                        { heading: 'Business Translation', points: ['A 0.5-point improvement in engagement (e.g., from 3.7 to 4.2 average across all employees at a unit) is associated with ~$50,150 in additional annual profit at that restaurant.', 'Across all 148 units: $50,150 × 148 = <strong>approximately $7.4 million</strong> in company-wide profit improvement.', 'This creates a powerful financial case: if a $1.2M engagement budget generates a 0.5-point improvement, the estimated ROI is ~6x.'] },
                ...
                },
                {
                    ...
                    answer: [
                        { heading: '📊 Expected Output', points: ['<strong>Payroll Cost Coefficient:</strong> approximately <span class="worked-number">−0.23</span>', '<strong>p-Value:</strong> ~0.003 — <strong>statistically significant</strong>', '<strong>Interpretation:</strong> A $1.00 increase in payroll reduces net profit by only about <strong>$0.23</strong> — not $1.00.'] },
                        { heading: 'Why Isn’t It −1.00?', points: ['If payroll were purely a cost with no benefit, every dollar spent would reduce profit by exactly $1.00 (coefficient = -1.0).', 'The fact that the coefficient is only -0.23 implies that every dollar of wages generates approximately <strong>$0.77 in offsetting revenue or efficiency gains</strong>.', 'Higher wages attract better workers → better service → higher sales → partially offsetting the cost increase.'] },
                        { heading: 'HR Implication', points: ['This is the economic argument for a living wage: increasing hourly pay is not a pure expense — a significant portion is recovered through higher productivity, lower turnover (fewer replacement costs), and better customer experience.', 'The data does not prove this definitively (the coefficient combines multiple effects), but it is consistent with this hypothesis.'] }
                    ],
            ...
            questions: [
                {
                    ...
                    answer: [
                        { heading: '📊 Expected Output', points: ['<strong>Odds Ratio (First Interview Score):</strong> approximately <span class="worked-number">3.50</span>', '<strong>p-Value:</strong> &lt; 0.001 — <strong>highly significant</strong>', '<strong>Interpretation:</strong> Each 1-point increase in first interview score increases the odds of being hired by approximately <strong>3.5 times</strong> — a very strong effect.'] },
                        { heading: 'What This Means', points: ['A candidate scoring 6 on the interview has roughly 3.5× the odds of being hired compared to a candidate scoring 5 — a very large effect on selection probability.', 'This is a strong and highly significant result. It tells us that the structured interview is doing its job as a selection tool: higher scores are powerfully associated with hire decisions.', 'For a <em>fair</em> process, we would want interview score to be the dominant predictor — and racial/gender identity to have an OR close to 1.0 (no effect). The high interview OR is a positive sign; see Q8 for whether race still matters after controlling for score.'] },
                        { heading: 'Odds vs. Probability', points: ['OR = 2.1 does <strong>not</strong> mean candidates with higher scores are "twice as likely" to be hired in percentage terms.', 'Odds = P(hired) / P(not hired). If baseline hire rate is 5%, baseline odds = 0.053. Multiplying by 2.1 gives 0.111, which translates to ~10% hire probability.', 'For Case Study 2, you do not need to convert — just interpret direction, magnitude, and significance.'] }
                    ],
                ...
                },
                {
                    ...
                    answer: [
                        { heading: '📊 Expected Output', points: ['<strong>Odds Ratio (White vs URM):</strong> approximately <span class="worked-number">1.24</span>', '<strong>p-Value:</strong> ~0.09 — <strong>not statistically significant</strong> at the standard α = 0.05 threshold', '<strong>Interpretation:</strong> Controlling for interview score only, White applicants have about 24% higher odds of being hired than URM applicants — but this difference <em>does not reach statistical significance</em> in this 2-predictor model.'] },
                        { heading: 'Does This Prove Discrimination?', points: ['In this simple 2-predictor model, race is not a statistically significant predictor of hiring once interview score is controlled.', 'The marginal OR of 1.24 suggests a small, positive advantage for White applicants — but with p≈0.09, we cannot rule out that this is due to random variation.', 'Important: this model only controls for <em>one</em> variable (interview score). Adding experience, second interview scores, and year controls may change the race OR substantially.'] },
                        { heading: 'What This Sets Up for Case Study 2', points: ['The strength of this 2-variable model is its simplicity — it isolates whether race matters beyond interview score alone.', 'In Case Study 2, you will run a fuller logistic model (with experience, interview 2, year dummies) that provides a more complete picture of potential hiring bias.', 'The analytical logic is identical — you are building toward the more comprehensive analysis by first understanding the core relationship between race and selection.'] }
                    ],
